    ...
    private CollidedSurface CheckSurroundings(bool forceCheck)
    {
        ...
        if (forceCheck || -vecToCheck.x >= -NEAR_ZERO)
        {
            ...
            if (closestHit.collider != null)
            {
                ...
                if (_collider2D.bounds.center.x - closestHit.centroid.x < minDistanceFromEnv/2f)
                {
                    transform.position += (minDistanceFromEnv/2f -
                        (_collider2D.bounds.center.x - closestHit.centroid.x)) *  new Vector3(-1,1);
                }
            }
        ...
        if (forceCheck || vecToCheck.x >= -NEAR_ZERO)
        {
            ...
            if (closestHit.collider != null)
            {
                ...
                if (closestHit.centroid.x - _collider2D.bounds.center.x < minDistanceFromEnv/2f)
                {
                    transform.position += ((minDistanceFromEnv/2f -
                                          (closestHit.centroid.x - _collider2D.bounds.center.x)) * new Vector3(1,1));
                    // transform.position += ((minDistanceFromEnv*2 -
                    //                         (closestHit.centroid.x - _collider2D.bounds.center.x)) * new Vector3(1,1))/2;
                }
            }
        }
